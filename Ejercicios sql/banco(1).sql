...
WHERE (select sum(m.importe) from movimiento m)>(c.saldo/5) AND s.cod_sucursal=c.cod_sucursal and m.cod_tipo_movimiento=tm.cod_tipo_movimiento and c.cod_cuenta=m.cod_cuenta
GROUP BY s.cod_sucursal, c.saldo, tm.descripcion;
<--20-->
SELECT s.cod_sucursal, c.saldo, tm.descripcion, sum(m.importe)
FROM sucursal s, cuenta c, tipo_movimiento tm, movimiento m 
WHERE (select sum(m.importe) from movimiento m)>(c.saldo/10) AND s.cod_sucursal=c.cod_sucursal and m.cod_tipo_movimiento=tm.cod_tipo_movimiento and c.cod_cuenta=m.cod_cuenta AND s.cod_sucursal!=4
GROUP BY s.cod_sucursal, c.saldo, tm.descripcion;


<--21-->
SELECT c.cod_cliente, c.apellidos, c.nombre, c.direccion
FROM cliente c, cuenta cu, sucursal s
WHERE (select sum(cu.saldo)from cuenta cu)>(s.CAPITAL_ANIO_ANTERIOR/5) and  c.cod_cliente=cu.cod_cliente and cu.cod_sucursal=s.cod_sucursal;
