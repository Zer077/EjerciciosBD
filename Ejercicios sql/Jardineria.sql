...
/*1*/
select c.nombre_cliente, e.nombre, e.apellido1, e.apellido2 from cliente c, empleado e where e.codigo_empleado=c.codigo_empleado_rep_ventas;
/*2*/
select c.nombre_cliente, e.nombre from cliente c inner join empleado e on e.codigo_empleado=c.codigo_empleado_rep_ventas inner join pago p on p.codigo_cliente=c.codigo_cliente;
/*3*/
select c.nombre_cliente, e.nombre from cliente c left join empleado e on e.codigo_empleado=c.codigo_empleado_rep_ventas left join pago p on p.codigo_cliente=c.codigo_cliente;
/*4*/
select c.nombre_cliente, e.nombre from cliente c, empleado e, oficina o where c.codigo_cliente in (select p.codigo_cliente from pago p where p.codigo_cliente=c.codigo_cliente) and o.codigo_oficina=e.codigo_oficina;
/*5*/
select c.nombre_cliente, e.nombre, o.pais from cliente c left join empleado e on e.codigo_empleado=c.codigo_empleado_rep_ventas left join oficina o on o.codigo_oficina=e.codigo_oficina left join pago p on p.codigo_cliente=c.codigo_cliente;
/*6*/
select Distinct o.linea_direccion1 from oficina o, empleado e, cliente c where c.ciudad='Fuenlabrada' and c.codigo_empleado_rep_ventas=e.codigo_empleado and e.codigo_oficina=o.codigo_oficina;
/*7*/
select c.nombre_cliente, e.nombre, o.pais from cliente c, empleado e, oficina o where  c.codigo_empleado_rep_ventas=e.codigo_empleado and e.codigo_oficina=o.codigo_oficina;  
/*8*/
--------------select e.nombre, e.nombre from empleado e where e.codigo_empleado=e.codigo_jefe; 
/*9*/
select distinct c.nombre_cliente from cliente c, pedido p where p.fecha_esperada<p.fecha_entrega and c.codigo_cliente=p.codigo_cliente;
/*10*/
select distinct c.nombre_cliente, p.gama from producto p, pedido pe, detalle_pedido de, cliente c where c. codigo_cliente=pe.codigo_cliente and de.codigo_pedido=pe.codigo_pedido and de.codigo_producto=p.codigo_producto;


/*Consulta multitabla externas*/

/*1*/
select distinct c.nombre_cliente from cliente c,  pago p where p.codigo_cliente is null and p.codigo_cliente(+)=c.codigo_cliente;

/*2*/
select distinct c.nombre_cliente from cliente c left join  pedido p on p.codigo_cliente=c.codigo_cliente;


/*3*/
select distinct c.nombre_cliente from cliente c left join  pedido p on p.codigo_cliente=c.codigo_cliente left join pago pa on pa.codigo_cliente=c.codigo_cliente;


/*4*/
select distinct e.nombre from empleado e left join oficina o on  e.codigo_oficina=o.codigo_oficina;


/*5*/
select distinct e.nombre from empleado e left join cliente c on  e.codigo_empleado=c.codigo_empleado_rep_ventas;


/*6*/
select distinct e.nombre from empleado e left join oficina o on  e.codigo_oficina=o.codigo_oficina left join cliente c on  e.codigo_empleado=c.codigo_empleado_rep_ventas;

/*7*/
select distinct p.nombre from producto p left join detalle_pedido de on  p.codigo_producto=de.codigo_producto left join pedido pe on pe.codigo_pedido=de.codigo_pedido;
...
/*9*/
select distinct c.nombre_cliente from cliente c left join pago p on p.codigo_cliente=c.codigo_cliente join  pedido pe on pe.codigo_cliente=c.codigo_cliente;
/*10*/
---
...
/*2*/
---select count(c.codigo_cliente), c.pais from cliente c order by c.pais;

/*3*/
select avg(total) from pago p where fecha_pago>='1/01/2009' and fecha_pago<='31/12/2009';

/*4*/
select count(codigo_pedido) num, estado from pedido p group by estado order by num desc;
/*5*/
...
/*9*/
select c.codigo_empleado_rep_ventas, count(c.codigo_cliente) from cliente group by codigo_empleado_rep_ventas;

/*10*/

select count(c.codigo_cliente) from cliente c, empleado e where c.codigo_empleado_rep_ventas(+)=e.codigo_empleado;


/*11*/
select max(p.fecha_pago), min(p.fecha_pago), c.nombre_cliente from pago p, cliente c where  p.codigo_cliente=c.codigo_cliente group by c.nombre_cliente; 

/*12*/
select distinct count(p.nombre), pe.codigo_pedido from producto p, pedido pe, detalle_pedido de where p.codigo_producto=de.codigo_producto and de.codigo_pedido=pe.codigo_pedido group by pe.codigo_pedido; 

/*13*/
select count(p.nombre), pe.codigo_pedido from producto p, pedido pe, detalle_pedido de where p.codigo_producto=de.codigo_producto and de.codigo_pedido=pe.codigo_pedido group by pe.codigo_pedido; 
/*14*/
select p.codigo_producto, count(dp.cantidad) 
from producto p, detalle_pedido dp
WHERE rownum <= 20 and p.codigo_producto=dp.codigo_producto
...
/*17*/
/*18*/
select sum(dp.cantidad), p.nombre, dp.cantidad*p.precio_venta,  dp.cantidad*p.precio_venta*0.21+dp.cantidad*p.precio_venta
from detalle_pedido dp, producto p
where dp.codigo_producto=p.codigo_producto
group by p.nombre, dp.cantidad*p.precio_venta,  dp.cantidad*p.precio_venta*0,21+dp.cantidad*p.precio_venta;


/*subconsulta*/
/*1*/
select c.nombre_cliente from cliente c where c.limite_credito>=(select MAX(cli.limite_credito) from cliente cli);
/*2*/
...
/*3*/
select p.nombre from producto p where p.precio_venta>=(select max(p1.precio_venta) from producto p1);
/*4*/--------

SELECT *
from (select p.nombre, count(dp.cantidad)
from producto p, detalle_pedido dp
where p.codigo_producto=dp.codigo_producto
group by p.nombre
order by count(dp.cantidad) desc) tabla1
where rownum=1;
/*5*/
select c.limite_credito, c.nombre_cliente
from cliente c
...
/*consulta variada*/
/*1*/
select c.nombre_cliente, count(p.nombre_pedido) 
...
group by c.nombre_cliente;
 ...
 from cliente c, pedido p
 where p.fecha_pedido>'01/01/2008' 
 and p.fecha_pedido>'31/12/2008' 
...
where o.codigo_oficina=e.codigo_oficina and e.codigo_empleado=c.codigo_empleado_rep_ventas and p.codigo_cliente(+)=c.codigo_cliente;
/*5*/
