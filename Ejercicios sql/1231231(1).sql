...
Usuario varchar2(20),
Fecha Date,
 constraint CTRL_PK primary key(tabla, usuario,fecha)
);
...
delete empleado where codigo_empleado=32;
...
drop trigger ins_empleado;
insert into empleado  VALUES (33,'Carlos','Soria','Jimenez','2444','csoria@jardineria.es','MAD-ES',3,'Director Oficina');



--Desarrollar un procedimiento que visualice el nombre, apellidos y puesto de todos los
--empleados, ordenados por primer apellido.
...
declare
    ...
    select nombre, apellido1, apellido2, puesto from empleado order by apellido1 asc;
    registro Cempleados%ROWTYPE;
begin
    open Cempleados;
        loop
            fetch Cempleados INTO registro;
            exit when Cempleados%notfound;
            DBMS_OUTPUT.PUT_LINE ('Nombre : ' || registro.nombre);
            DBMS_OUTPUT.PUT_LINE ('1er Apellido: ' || registro.apellido1);
            DBMS_OUTPUT.PUT_LINE ('2º Apellido: ' || registro.apellido2);
...
--Desarrollar un procedimiento que muestre el código de cada oficina y el número de
--empleados que tiene.


declare
    cursor puestos is
    select o.codigo_oficina as oficina,count(e.codigo_empleado) as empleado from oficina o, empleado e where o.codigo_oficina=e.codigo_oficina group by o.codigo_oficina;
    registro puestos%ROWTYPE;
begin
    open puestos;
        loop
            fetch puestos INTO registro;
            exit when puestos%notfound;
            DBMS_OUTPUT.PUT_LINE ('Puesto : ' || registro.oficina);
            DBMS_OUTPUT.PUT_LINE ('nº Empleado: ' || registro.empleado);
            DBMS_OUTPUT.PUT_LINE (' ---- ');
        END LOOP;
    Close puestos;
END;
...
--- Desarrollar un procedimiento que reciba como parámetro una cadena de texto y muestre el
--código de cliente y nombre de cliente de todos los clientes cuyo nombre contenga la cadena
--especificada. Al finalizar debe mostrar el número de clientes mostrados.
create or replace procedure clienteCadena(cadena varchar2)
is
   
    cursor puestos is
    select c.codigo_cliente, c.nombre_cliente  from cliente c where c.nombre_cliente='%||cadena||%';
    registro puestos%ROWTYPE;
begin 
    open puestos;
        loop
            fetch puestos INTO registro;
            exit when puestos%notfound;
            DBMS_OUTPUT.PUT_LINE ('nombre: ' || registro.nombre_cliente);
            DBMS_OUTPUT.PUT_LINE ('codigo: ' || registro.codigo_cliente);
            DBMS_OUTPUT.PUT_LINE (' ---- ');
        END LOOP;
    Close puestos;
END clienteCadena;
...
is
    ...
    cursor Cproducto is
    select  p.codigo_producto as codigo, p.nombre as nombre, p.gama as gama from producto p, detalle_pedido dp where p.codigo_producto=dp.codigo_producto order by dp.cantidad;
    registro Cproducto%ROWTYPE;
begin 
    open Cproducto;
        for i in 1..5 loop
            fetch Cproducto INTO registro;
            exit when Cproducto%notfound;
            DBMS_OUTPUT.PUT_LINE ('nombre: ' || registro.nombre);
            DBMS_OUTPUT.PUT_LINE ('codigo: ' || registro.codigo);
...
--Desarrollar un procedimiento que aumente el límite de crédito en un 50% a aquellos clientes
--que cuyo valor sea inferior al límite de crédito medio actual.

create or replace procedure ej5
is
    cursor Cproducto is
    select limite_credito as limite from cliente c where c.limite_credito<(select sum(limite_credito)/count(codigo_cliente) from cliente);
    registro Cproducto%ROWTYPE;
begin 
    open Cproducto;
        for i in 1..5 loop
            fetch Cproducto INTO registro;
            exit when Cproducto%notfound;
            DBMS_OUTPUT.PUT_LINE ('credito: ' || registro.limite*1.5);     
            DBMS_OUTPUT.PUT_LINE (' ---- ');
        END LOOP;
    Close Cproducto;
END ej5;
...
--Diseñar una función que dado un código de producto retorne el número de unidades
--vendidas del producto. En caso de introducir un producto inexistente retornará -1

create or replace procedure ej6(codigo varchar2)
is
    cursor Cproducto is
        select p.codigo_producto, sum(dp.cantidad) from producto p, detalle_pedido dp where dp.codigo_producto=p.codigo_producto and codigo_producto='codigo' group by p.codigo_producto;
    registro Cproducto%ROWTYPE;
begin 
    open Cproducto;
        for i in 1..5 loop
            fetch Cproducto INTO registro;
            exit when Cproducto%notfound;
            DBMS_OUTPUT.PUT_LINE ('COD: ' || registro.codigo_producto);
...
-- Diseñar una función que dado un código de cliente y un año retorne la cantidad total pagada
--por el cliente durante el periodo indicado. En caso de introducir un cliente inexistente retornará  -1.
create or replace procedure ej7(codigo varchar2, anio number)
is
    cursor Cproducto is
        select sum(p.total) as total, c.codigo_cliente as cliente from cliente c, pago p where p.codigo_cliente=c.codigo_cliente and c.codigo_cliente='codigo' and p.fecha_pago>'01-01-'||'anio' and p.fecha_pago<'31-12-'||'anio' group by c.codigo_cliente;
    registro Cproducto%ROWTYPE;
begin 
    open Cproducto;
        for i in 1..5 loop
            fetch Cproducto INTO registro;
            exit when Cproducto%notfound;
            DBMS_OUTPUT.PUT_LINE ('COD: ' || registro.total);
            DBMS_OUTPUT.PUT_LINE ('cantidad: ' || registro.cliente);
            DBMS_OUTPUT.PUT_LINE (' ---- ');
        END LOOP;
    Close Cproducto;
END ej7;
...
create or replace procedure ej8(codigo varchar2) is
begin 
delete pedido where codigo_cliente=codigo;
    DBMS_OUTPUT.PUT_LINE ('num: ' || sql%rowcount);
    DBMS_OUTPUT.PUT_LINE (' ---- ');
END ej8;
/

--Desarrollar un procedimiento que actualice el precio de venta de todos los productos de una
--determinada gama. El procedimiento recibirá como parámetro la gama, y actualizará el precio
--de venta como el precio de proveedor + 70%. Al finalizar el procedimiento indicará el número
--de productos actualizados y su gama.

create or replace procedure ej9(cgama varchar2)
is
    cursor Cproducto is
        select precio_venta as venta, precio_proveedor as proveedor  from  producto where gama=cgama;
    registro Cproducto%ROWTYPE;
    
    productos producto%rowtype
    
begin 
update precio_venta into producto
    open Cproducto;
            exit when Cproducto%notfound;
...
create or replace trigger actualizar
    ...
    for each row
begin
    update producto set cantidad_en_stock=cantidad_en_stock-:new.cantidad where codigo_producto=:new.codigo_producto;
END ACTUALIZAR;
/
...
create or replace trigger ej2
    ...
    for each row
begin
    insert into auditoria values (user||'-'||sysdate||'-'||:new.codigo_cliente);
end ej2;
/
...
begin
    IF DELETING THEN
         insert into auditoria values (user||'-'||sysdate||'-'|| 'eliminar'  ||'-'||:new.codigo_cliente);
    ELSIF INSERTING THEN
        insert into auditoria values (user||'-'||sysdate||'-'|| 'insertar'  ||'-'||:new.codigo_cliente);
    ELSE
        insert into auditoria values (user||'-'||sysdate||'-'|| 'update'  ||'-'||:new.codigo_cliente);
    END IF;
...
declare
    cursor media is
    select avg(limite_credito) as limite from cliente c, empleado e where e.codigo_empleado=c.codigo_empleado_rep_ventas  and :new.codigo_empleado_rep_ventas=e.codigo_empleado group by e.codigo_empleado;
    registro media%ROWTYPE;   
    mi_excepcion EXCEPTION;
begin
    open media;
        loop
            FETCH media INTO registro;
                 if :new.limite_credito>=registro.limite THEN
                         RAISE mi_excepcion;
                    end if;
        end loop;
    close media;
...
create or replace trigger ej5
   after update on oficina
       for each row

declare
   
begin
 update empleado e set e.codigo_oficina=:new.codigo_oficina where e.codigo_oficina=:old.codigo_oficina;
...
delete detalle_pedido where codigo_pedido=:old.codigo_pedido;


end ej6;
/
 drop table balance_anual;
...
alter table detalle_pedido add anio number(38);
update detalle_pedido set  anio=2019;

insert into balance_anual (anio) values (2019);
...
productos producto%rowtype;
begin
select * into productos from producto p where codigo_producto=:new.codigo_producto;
update into balance_anual b set ingresos=ingresos+(productos.precio_venta*:new.cantidad) where b.anio=:new.anio;
update into balance_anual b set gastos=gastos+(productos.precio_proveedor*:new.cantidad) where b.anio=:new.anio;
...
is
  cursor productos is
  select * from producto where gama=cgama;
...
update producto set precio_venta=registro.precio_proveedor+(registro.precio_proveedor*0.7) where registro.codigo_producto=codigo_producto;

end loop;
DBMS_OUTPUT.PUT_LINE (productos%rowcount);
DBMS_OUTPUT.PUT_LINE (cgama);
