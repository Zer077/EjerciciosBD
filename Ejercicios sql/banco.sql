...
<--2-->
SELECT cod_cuenta, MAX(saldo), MIN(saldo), AVG(saldo)
FROM cuenta
Group by cod_cuenta;
...
<--7-->
 ...
 FROM  cuenta
 WHERE (INTERES>10 AND COD_SUCURSAL=1 OR COD_SUCURSAL=2);
<--8-->
...
<--11-->
SELECT tm.descripcion, SUM(m.importe), m.cod_tipo_movimiento
...
<--12-->
SELECT s.cod_sucursal, sum(m.importe)
FROM SUCURSAL S, movimiento m, cuenta c
WHERE s.direccion='Paseo Castellana' and c.cod_cuenta=m.cod_cuenta and c.cod_sucursal=s.cod_sucursal
...
WHERE c.cod_cuenta=m.cod_cuenta AND m.cod_cuenta=null;
...
group by c.cod_cliente;
<--16-->
SELECT c.cod_cliente, sum(cu.saldo) as "total", count(cu.cod_cuenta) as "cuenta"
FROM cuenta cu, cliente c
WHERE cu.saldo>35000
group by c.cod_cliente;
