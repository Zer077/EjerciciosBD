...
WHERE (cu.cod_cliente=c.cod_cliente AND m.cod_tipo_movimiento=t.cod_tipo_movimiento AND m.cod_cuenta=cu.cod_cuenta AND t.descripcion='Retirada  por cajero autmático');

SELECT c.nombre, AVG(m.importe)
FROM movimiento m, tipo_movimiento t, cliente c, cuenta cu
where T.DESCRIPCION='Retirada por cajero automático'
...
Where ((tm.SALIDA='Sí') AND (M.COD_TIPO_MOVIMIENTO=TM.COD_TIPO_MOVIMIENTO))
...
WHERE t.salida='Sí' AND s.direccion='Paseo Castellana%' and c.cod_cuenta=m.cod_cuenta and c.cod_sucursal=s.cod_sucursal and m.cod_tipo_movimiento=t.cod_tipo_movimiento;
<--13-->
...
INSERT INTO CLIENTE (cod_cliente, apellidos, nombre, direccion) VALUES ('FJUAREZ', 'Fernando', 'Juarez Marquez', 'C/ Mata 11, 1ºF');
...
/*3*/
INSERT INTO CLIENTE (cod_cliente, apellidos, nombre, direccion) VALUES ('FRaton', 'Raton Romero', 'Fernando', 'C/Nazis nº666');
...
INSERT INTO Movimiento_Salida (cuenta, mes, num_mov_mes, cod_tipo_mov, importe, fecha_hora) VALUES Select m.cod_cuenta, m.mes, m.num_mov_mes, m.cod_tipo_movimiento, m.importe, m.fecha_hora from movimiento m, tipo_movimiento tm where tm.cod_tipo_mov=m.cod.tipo_movimiento tm.salis like 'Sí'; 
...
Insert into movimiento_entrada (cod_cuenta, mes, num_mov_mes, cod_tipo_movimiento, importe, fecha_hora) VALUES select m.cod_cuenta,m.mes, m.num_mov_mes, m.cod_tipo_movimiento, m.importe, m.fecha_hora from movimiento m, tipo_movimiento tm where tm.salida="Sí" and tm.cod_tipo_movimiento=m=cod_tipo_movimiento
/*13*/
